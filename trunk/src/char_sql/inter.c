...
int inter_accreg_tosql(int account_id, int char_id, struct accreg* reg, int type)
{
	...
	case 2: //Account Reg
		...
		break;
	case 1: //Account2 Reg
		ShowError("inter_accreg_tosql: Char server nao deveria utilizar valores de registro do tipo 1 (##). Isso e trabalho do login server!\n");
		return 0;
	default:
		ShowError("inter_accreg_tosql: tipo %d invalido\n", type);
		return 0;
	}
...
int inter_accreg_fromsql(int account_id,int char_id, struct accreg *reg, int type)
{
	...
	case 2: //account reg
		...
		break;
	case 1: //account2 reg
		ShowError("inter_accreg_fromsql: Char server nao deveria utilizar valores de registro do tipo 1 (##). Isso e trabalho do login server!\n");
		return 0;
	default:
		ShowError("inter_accreg_fromsql: Tipo %d invalido\n", type);
		return 0;
	}
...
static int inter_config_read(const char* cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(!strcmpi(w1,"char_server_ip")) {
			...
			ShowStatus ("configurar char_server_ip : %s\n", w2);
		} else
		if(!strcmpi(w1,"char_server_port")) {
			char_server_port = atoi(w2);
			ShowStatus ("configurar char_server_port : %s\n", w2);
		} else
		if(!strcmpi(w1,"char_server_id")) {
			strcpy(char_server_id,w2);
			ShowStatus ("configurar char_server_id : %s\n", w2);
		} else
		if(!strcmpi(w1,"char_server_pw")) {
			strcpy(char_server_pw,w2);
			ShowStatus ("configurar char_server_pw : %s\n", w2);
		} else
		if(!strcmpi(w1,"char_server_db")) {
			strcpy(char_server_db,w2);
			ShowStatus ("configurar char_server_db : %s\n", w2);
		} else
		if(!strcmpi(w1,"default_codepage")) {
			strcpy(default_codepage,w2);
			ShowStatus ("configurar default_codepage : %s\n", w2);
		}
#ifndef TXT_SQL_CONVERT
	...
	ShowInfo ("Carregamento de %s finalizado.\n", cfgName);
...
int inter_init_sql(const char *file)
{
	...
	ShowInfo("Conectando ao Banco de Dados do Character server.... (Character Server)\n");
...
int mapif_account_reg_reply(int fd,int account_id,int char_id, int type)
{
	...
	if(reg->reg_num==0){
		...
		WFIFOW(fd,2)=p;
		if (p>= 5000)
			ShowWarning("Muitos registros para a conta %d:%d, alguns valores nao foram carregados.\n", account_id, char_id);
	}
...
int check_ttl_wisdata(void)
{
	...
	do {
		...
		for(i = 0; i < wis_delnum; i++) {
			...
			ShowWarning("inter: dados wis id=%d time out : de %s para %s\n", wd->id, wd->src, wd->dst);
