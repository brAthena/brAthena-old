...
{	
	...
	if (!sd)
	{
		ShowError("chrif_load_skill_reuse_data: Personagem %d n„o encontrado!\n", aid);
		return -1;
	}
	if (sd->status.char_id != cid)
	{
		ShowError("chrif_load_skill_reuse_data: Recebendo dados da conta %d, a ID do char n„o È a mesma (%d != %d)!\n", aid, sd->status.char_id, cid);
		return -1;
	}
	count = RFIFOW(fd,12);
	for (i = 0; i < count; i++)
