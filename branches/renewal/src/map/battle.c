...
static int battle_calc_base_damage(struct status_data *status, struct weapon_atk *wa, struct status_change *sc, unsigned short t_size, struct map_session_data *sd, int flag)
{
	...
	//BaseATK * 2 + RefineATK * Size * EquipmentATK = Nova fmula do RE. [Protimus]
...
static struct Damage battle_calc_weapon_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int wflag)
{
	...
	if (flag.cri)
	{
		...
		if(sc && sc->data[SC_BENEDICTIO] && (battle_check_undead(tstatus->race, tstatus->def_ele) || tstatus->race == RC_DEMON))
			flag.idef = flag.idef2 = 1; //SC_BENEDICTIO ignorar defesa de Morto-Vivo e Demios [Floozie]
		if( !flag.hit )
			switch(skill_num)
			{
				case AS_SPLASHER:
...
#define ATK_ADD2( a , b ) { wd.damage+= a; if (flag.lh) wd.damage2+= b; }
		...
		{	//Defense reduction
			...
			{
				ATK_RATE2(
					flag.pdef  ? flag.pdef  * (def1 + vit_def) : def_rate,
					flag.pdef2 ? flag.pdef2 * (def1 + vit_def) : def_rate
				);
