...
void pc_paycash(struct map_session_data *sd, int price, int points)
{
	...
	sprintf(output, "VocÍ utilizou %d Pontos Kafra e %d ROPs. VocÍ ainda possui %d Pontos Kafra e %d ROPs.", points, cash, sd->kafraPoints, sd->cashPoints);
...
void pc_getcash(struct map_session_data *sd, int cash, int points)
{
	...
	if( cash > 0 )
	{
		...
		sprintf(output, "VocÍ recebeu %d ROPs. Total: %d ROPs.", cash, sd->cashPoints);
		clif_disp_onlyself(sd, output, strlen(output));
	}
	...
	if( points > 0 )
	{
		...
		sprintf(output, "VocÍ recebeu %d Pontos Kafra. Total: %d Pontos Kafra.", points, sd->kafraPoints);
		clif_disp_onlyself(sd, output, strlen(output));
	}
}
...
int pc_jobid2mapid(unsigned short b_class)
{
	...
	// 3-2 advanced
		...
		case JOB_MINSTREL_T: 		return MAPID_MINSTRELWANDERER;
		case JOB_WANDERER_T:		return MAPID_MINSTRELWANDERER;
