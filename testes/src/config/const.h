...
/* Cálculos de ATCMD_FUNC(mobinfo) HIT e FLEE */
	...
	#define RE_SKILL_REDUCTION(){ \
			wd.damage = battle_calc_elefix(src, target, skill_id, skill_lv, battle_calc_cardfix(BF_WEAPON, src, target, nk, s_ele, s_ele_, wd.damage, 2, wd.flag), nk, n_ele, s_ele, s_ele_, false, flag.arrow); \
			if(flag.lh) \
				wd.damage2 = battle_calc_elefix(src, target, skill_id, skill_lv, battle_calc_cardfix(BF_WEAPON, src, target, nk, s_ele, s_ele_, wd.damage2, 3, wd.flag), nk, n_ele, s_ele, s_ele_, true, flag.arrow); \
		}
#else
