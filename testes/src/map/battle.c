...
int battle_calc_elefix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int damage, int nk, int n_ele, int s_ele, int s_ele_, bool left, int flag){
	...
	{ // Descriptions indicate this means adding a percent of a normal attack in another element. [Skotlex]
			damage = 
#ifndef RENEWAL
				battle_calc_base_damage(sstatus, &sstatus->rhw, sc, tstatus->size, ((TBL_PC*)src), (flag?2:0))
#else
			...
			if(left) {
				damage = 
#ifndef RENEWAL
					battle_calc_base_damage(sstatus, &sstatus->lhw, sc, tstatus->size, ((TBL_PC*)src), (flag?2:0))
#else
