...
int battle_calc_weapon_damage(struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, struct weapon_atk *watk, int nk, bool n_ele, short s_ele, short s_ele_, int size, int type, int flag, int flag2){ // [malufett]
	...
	*/
	...
	if(sc && sc->data[SC_WATK_ELEMENT])
		...
		damage = battle_calc_elefix(src, bl, skill_id, skill_lv, damage + eatk, nk, n_ele, s_ele, s_ele_, false, flag);
	...
	**/
	...
	if(type == EQI_HAND_R)
		damage = battle_calc_cardfix(BF_WEAPON, src, bl, nk, s_ele, s_ele_, damage, 2, flag2);
	else
		damage = battle_calc_cardfix(BF_WEAPON, src, bl, nk, s_ele, s_ele_, damage, 3, flag2);
...
int battle_calc_base_damage(struct block_list *src, struct block_list *bl, uint16 skill_id, uint16 skill_lv, int nk, bool n_ele, short s_ele, short s_ele_, int type, int flag, int flag2)
{
	...
	if(sc && sc->data[SC_TK_SEVENWIND] && !sc->data[SC_WATK_ELEMENT])
		batk = battle_calc_elefix(src, bl, skill_id, skill_lv, status->batk, nk, n_ele, s_ele, s_ele_, false, flag);
	else
		batk = status->batk;
...
int battle_calc_masteryfix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int damage, int div, bool left, bool weapon){
	...
	switch(skill_id) { // specific skill masteries
		...
		case CR_ACIDDEMONSTRATION:
			return damage;
		case NJ_SYURIKEN:
			if((skill = pc_checkskill(sd,NJ_TOBIDOUGU)) > 0 && weapon)
				damage += 3 * skill;
			break;	
		case NJ_KUNAI:
			if(weapon)
...
int battle_calc_elefix(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, int damage, int nk, int n_ele, int s_ele, int s_ele_, bool left, int flag){
	...
	{ // Descriptions indicate this means adding a percent of a normal attack in another element. [Skotlex]
			damage = 
#ifndef RENEWAL
				battle_calc_base_damage(sstatus, &sstatus->rhw, sc, tstatus->size, BL_CAST(BL_PC, src), (flag?2:0))
#else
				battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_R, (flag?2:0)|(sc && sc->data[SC_MAXIMIZEPOWER]?1:0)|(sc && sc->data[SC_WEAPONPERFECT]?8:0), 0)
#endif
			...
			if(left) {
				damage = 
#ifndef RENEWAL
					battle_calc_base_damage(sstatus, &sstatus->lhw, sc, tstatus->size, BL_CAST(BL_PC, src), (flag?2:0))
#else
					battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, (flag?2:0)|(sc && sc->data[SC_MAXIMIZEPOWER]?1:0)|(sc && sc->data[SC_WEAPONPERFECT]?8:0), 0)
#endif
					* sc->data[SC_WATK_ELEMENT]->val2 / 100;
				damage += battle_attr_fix(src, target, damage, sc->data[SC_WATK_ELEMENT]->val1, tstatus->def_ele, tstatus->ele_lv);
			}
		...
		case BF_WEAPON:
			switch( skill_id )
			{
				...
				case RK_CRUSHSTRIKE:
					...
					break;
				case RK_STORMBLAST:
					skillratio += -100 + 100 * (sd ? pc_checkskill(sd,RK_RUNEMASTERY) : 1) +  100 * (status_get_int(src) / 4);
					break;
		...
		{	//Calc base damage according to skill
            ...
            default: {
				...
				if (skill_id == WZ_FIREPILLAR)
					MATK_ADD(50);
				if( sd && (i=pc_checkskill(sd,AB_EUCHARISTICA)) > 0 &&
		...
		switch(skill_id) {
			...
			case NJ_ISSEN: // [malufett]
...
#else
				{
					short totaldef = status_get_total_def(target);
					i = 0;
					wd.damage = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_R, (sc && sc->data[SC_MAXIMIZEPOWER]?1:0)|(sc && sc->data[SC_WEAPONPERFECT]?8:0), wd.flag);
...
#ifdef RENEWAL
				wd.damage = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_R, i, wd.flag);
				wd.damage = battle_calc_masteryfix(src, target, skill_id, skill_lv, wd.damage, wd.div_, 0, flag.weapon);
				if (flag.lh) {
					wd.damage2 = battle_calc_base_damage(src, target, skill_id, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, i, wd.flag);
