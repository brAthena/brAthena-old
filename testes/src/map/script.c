...
const char* parse_variable(const char* p) {
	...
	{// cannot assign a variable which exists as a function or label
		disp_error_message("N„o È possÌvel modificar uma vari·vel que tem o mesmo nome de uma funÁ„o ou um label.", p);
	}
	...
	if( p2 ) {// process the variable index
		...
		if( *p3 != ']' ) {// closing parenthesis is required for this script
			disp_error_message("Falta de fechamento ']' parÍntese para a atribuiÁ„o de vari·vel.", p3);
		}
