...
int unit_attack(struct block_list *src,int target_id,int continuous)
{
	...
	if( src->type == BL_PC ) {
		...
		if( pc_is90overweight(sd) || pc_isridingwug(sd) || battle_config.gm_cant_attack && pc_get_group_id(sd) > 3 && (target->type == BL_PC || target->type == BL_MOB) ) { // Acima do peso - Montado no Lobo - Proibi ataque de contas [GM/ADM] -  [Holy]
			unit_stop_attack(src);
			return 0;
		}
	}
