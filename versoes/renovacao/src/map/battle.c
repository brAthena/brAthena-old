...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv,int element)
{
	...
	if( sc && sc->count )
	{
		...
		if( (sce=sc->data[SC_LIGHTNINGWALK]) && flag&BF_LONG && damage>0 && rand()%100 < sce->val3 ) {
			skill_blown(src,bl,distance_bl(src,bl)-1,map_calc_dir(src,bl->x,bl->y),0);
			d->div_ = ATK_DEF;
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_3rd_aspd",                       &battle_config.max_3rd_aspd,                    199,    100,    199,            },
...
void battle_adjust_conf()
{
	...
	battle_config.max_3rd_aspd = 2000 - battle_config.max_3rd_aspd*10;
