...
static struct {
	...
	} curly[256];		// \89E\83J\83b\83R\82̏\EE\95\F1
	int curly_count;	// \89E\83J\83b\83R\82̐\94
	int index;			// \83X\83N\83\8A\83v\83g\93\E0\82Ŏg\97p\82\B5\82\BD\8D\\95\B6\82̐\94
} syntax;
...
/*==========================================
 * \83\8D\81[\83J\83\8B\83v\83\8D\83g\83^\83C\83v\90錾 (\95K\97v\82ȕ\A8\82̂\DD)
 *------------------------------------------*/
...
/*==========================================
 * \83G\83\89\81[\83\81\83b\83Z\81[\83W\8Fo\97\CD
 *------------------------------------------*/
...
static void add_scriptl(int l)
{
	...
	case C_NOP:
	case C_USERFUNC:
		// \83\89\83x\83\8B\82̉\\90\AB\82\AA\82\A0\82\E9\82̂\C5backpatch\97p\83f\81[\83^\96\84\82ߍ\9E\82\DD
		add_scriptc(C_NAME);
...
/*==========================================
 * \83\89\83x\83\8B\82\F0\89\F0\8C\88\82\B7\82\E9
 *------------------------------------------*/
...
const char* parse_line(const char* p)
{
	...
	p=skip_space(p);
	if(*p==';') {
		// if(); for(); while(); \82̂\BD\82߂ɕ\B6\94\BB\92\E8
	...
	// if, for , while \82̕\B6\94\BB\92\E8
	p = parse_syntax_close(p+1);
...
const char* parse_curly_close(const char* p)
{
	if(syntax.curly_count <= 0) {
		...
		// switch() \95\B6\94\BB\92\E8
		int pos = syntax.curly_count-1;
		char label[256];
		int l;
		// \88ꎞ\95ϐ\94\82\F0\8F\C1\82\B7
		sprintf(label,"set $@__SW%x_VAL,0;",syntax.curly[pos].index);
		syntax.curly[syntax.curly_count++].type = TYPE_NULL;
		parse_line(label);
		syntax.curly_count--;

		// \96\B3\8F\F0\8C\8F\82ŏI\97\B9\83|\83C\83\93\83^\82Ɉړ\AE
		sprintf(label,"goto __SW%x_FIN;",syntax.curly[pos].index);
		syntax.curly[syntax.curly_count++].type = TYPE_NULL;
		parse_line(label);
		syntax.curly_count--;

		// \8C\BB\8Dݒn\82̃\89\83x\83\8B\82\F0\95t\82\AF\82\E9
		sprintf(label,"__SW%x_%x",syntax.curly[pos].index,syntax.curly[pos].count);
		l=add_str(label);
		set_label(l,script_pos, p);

		if(syntax.curly[pos].flag) {
			// default \82\AA\91\B6\8D݂\B7\82\E9
		...
		// \8FI\97\B9\83\89\83x\83\8B\82\F0\95t\82\AF\82\E9
...
//	 if, switch, while \82\F0\82\B1\82̓\E0\95\94\82ŏ\88\97\9D\82\B5\82܂\B7\81B
const char* parse_syntax(const char* p)
{
		...
		if(p2 - p == 4 && !strncasecmp(p,"case",4)) {
			...
			if(pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
				...
				if(syntax.curly[pos].count != 1) {
					// FALLTHRU \97p\82̃W\83\83\83\93\83v
				...
				syntax.curly[syntax.curly_count++].type = TYPE_NULL;
				// \82Q\89\F1parse \82\B5\82Ȃ\A2\82ƃ_\83\81
				p2 = parse_line(label);
				parse_line(p2);
				syntax.curly_count--;
				if(syntax.curly[pos].count != 1) {
					// FALLTHRU \8FI\97\B9\8C\E3\82̃\89\83x\83\8B
			...
			// continue \82̏\88\97\9D
			char label[256];
			int pos = syntax.curly_count - 1;
			while(pos >= 0) {
				if(syntax.curly[pos].type == TYPE_DO) {
					sprintf(label,"goto __DO%x_NXT;",syntax.curly[pos].index);
					syntax.curly[pos].flag = 1; // continue \97p\82̃\8A\83\93\83N\92\A3\82\E9\83t\83\89\83O
					break;
				} else if(syntax.curly[pos].type == TYPE_FOR) {
	...
	case 'D':
		if(p2 - p == 7 && !strncasecmp(p,"default",7)) {
			...
			if(pos < 0 || syntax.curly[pos].type != TYPE_SWITCH) {
				...
				// \96\B3\8F\F0\8C\8F\82Ŏ\9F\82̃\8A\83\93\83N\82ɔ\F2\82΂\B7
				sprintf(label,"goto __SW%x_%x;",syntax.curly[pos].index,syntax.curly[pos].count+1);
				syntax.curly[syntax.curly_count++].type = TYPE_NULL;
				parse_line(label);
				syntax.curly_count--;

				// default \82̃\89\83x\83\8B\82\F0\95t\82\AF\82\E9
			...
			// \8C\BB\8Dݒn\82̃\89\83x\83\8B\8C`\90\AC\82\B7\82\E9
	...
	case 'F':
		if(p2 - p == 3 && !strncasecmp(p,"for",3)) {
			...
			// \8F\89\8A\FA\89\BB\95\B6\82\F0\8E\C0\8Ds\82\B7\82\E9
			syntax.curly[syntax.curly_count++].type = TYPE_NULL;
			p=parse_line(p);
			syntax.curly_count--;

			// \8F\F0\8C\8F\94\BB\92f\8AJ\8En\82̃\89\83x\83\8B\8C`\90\AC\82\B7\82\E9
			...
			p=skip_space(p);
			if(*p == ';') {
				// for(;;) \82̃p\83^\81[\83\93\82Ȃ̂ŕK\82\B8\90^
				;
			} else {
				// \8F\F0\8C\8F\82\AA\8BU\82Ȃ\E7\8FI\97\B9\92n\93_\82ɔ\F2\82΂\B7
			...
			// \83\8B\81[\83v\8AJ\8En\82ɔ\F2\82΂\B7
			sprintf(label,"goto __FR%x_BGN;",syntax.curly[pos].index);
			syntax.curly[syntax.curly_count++].type = TYPE_NULL;
			parse_line(label);
			syntax.curly_count--;

			// \8E\9F\82̃\8B\81[\83v\82ւ̃\89\83x\83\8B\8C`\90\AC\82\B7\82\E9
			sprintf(label,"__FR%x_NXT",syntax.curly[pos].index);
			l=add_str(label);
			set_label(l,script_pos,p);

			// \8E\9F\82̃\8B\81[\83v\82ɓ\FC\82鎞\82̏\88\97\9D
			// for \8DŌ\E3\82\CC ')' \82\F0 ';' \82Ƃ\B5\82Ĉ\B5\82\A4\83t\83\89\83O
			...
			// \8F\F0\8C\8F\94\BB\92菈\97\9D\82ɔ\F2\82΂\B7
			sprintf(label,"goto __FR%x_J;",syntax.curly[pos].index);
			syntax.curly[syntax.curly_count++].type = TYPE_NULL;
			parse_line(label);
			syntax.curly_count--;

			// \83\8B\81[\83v\8AJ\8En\82̃\89\83x\83\8B\95t\82\AF
...
const char* parse_syntax_close(const char *p) {
	// if(...) for(...) hoge(); \82̂悤\82ɁA\82P\93x\95\B6\82\E7\82ꂽ\82\E7\8Dēx\95\B6\82\E7\82\EA\82邩\8Am\94F\82\B7\82\E9
	int flag;
...
// if, for , while , do \82̕\B6\94\BB\92\E8
//	 flag == 1 : \95\B6\82\E7\82ꂽ
//	 flag == 0 : \95\B6\82\E7\82\EA\82Ȃ\A2
const char* parse_syntax_close_sub(const char* p,int* flag)
{
	char label[256];
	int pos = syntax.curly_count - 1;
	...
	*flag = 1;
	...
	if(syntax.curly_count <= 0) {
		*flag = 0;
		...
		// if \95\B6
		syntax.curly_count--;
		// \8DŏI\92n\82̃\89\83x\83\8B\82\F0\95t\82\AF\82\E9
		sprintf(label,"__IF%x_FIN",syntax.curly[pos].index);
		l=add_str(label);
		set_label(l,script_pos,p);
		if(syntax.curly[pos].flag == 1) {
			// \82\B1\82\CCif\82ɑ΂\B7\82\E9else\82\B6\82\E1\82Ȃ\A2\82̂Ń|\83C\83\93\83^\82̈ʒu\82͓\AF\82\B6
			return bp;
		}
		return p;
		...
		if(syntax.curly[pos].flag) {
			// \8C\BB\8Dݒn\82̃\89\83x\83\8B\8C`\90\AC\82\B7\82\E9(continue \82ł\B1\82\B1\82ɗ\88\82\E9)
		...
		// \8AJ\8En\92n\93_\82ɔ\F2\82΂\B7
		sprintf(label,"goto __DO%x_BGN;",syntax.curly[pos].index);
		syntax.curly[syntax.curly_count++].type = TYPE_NULL;
		parse_line(label);
		syntax.curly_count--;

		// \8F\F0\8C\8F\8FI\97\B9\92n\93_\82̃\89\83x\83\8B\8C`\90\AC\82\B7\82\E9
		sprintf(label,"__DO%x_FIN",syntax.curly[pos].index);
		l=add_str(label);
		set_label(l,script_pos,p);
		p = skip_space(p);
		...
		// \8E\9F\82̃\8B\81[\83v\82ɔ\F2\82΂\B7
		sprintf(label,"goto __FR%x_NXT;",syntax.curly[pos].index);
		syntax.curly[syntax.curly_count++].type = TYPE_NULL;
		parse_line(label);
		syntax.curly_count--;

		// for \8FI\97\B9\82̃\89\83x\83\8B\95t\82\AF
		sprintf(label,"__FR%x_FIN",syntax.curly[pos].index);
		l=add_str(label);
		set_label(l,script_pos,p);
		syntax.curly_count--;
		...
		// while \8F\F0\8C\8F\94\BB\92f\82֔\F2\82΂\B7
		sprintf(label,"goto __WL%x_NXT;",syntax.curly[pos].index);
		syntax.curly[syntax.curly_count++].type = TYPE_NULL;
		parse_line(label);
		syntax.curly_count--;

		// while \8FI\97\B9\82̃\89\83x\83\8B\95t\82\AF
...
/*==========================================
 * \92萔\83f\81[\83^\83x\81[\83X\82̓ǂݍ\9E\82\DD
 *------------------------------------------*/
...
{
	// \83G\83\89\81[\82\AA\94\AD\90\B6\82\B5\82\BD\8Ds\82\F0\8B\81\82߂\E9
	int j;
	int line = start_line;
	const char *p;
...
/*==========================================
 * \83X\83N\83\8A\83v\83g\82̉\F0\90\CD
 *------------------------------------------*/
struct script_code* parse_script(const char *src, const char *file,int line,int options)
{
	...
	while( syntax.curly_count != 0 || *p != end )
	{
		...
		// label\82\BE\82\AF\93\C1\8Eꏈ\97\9D
		...
		// \91\BC\82͑S\95\94\88ꏏ\82\AD\82\BD
		p=parse_line(p);
		p=skip_space(p);
		...
		case '.':
			{
				struct linkdb_node** n =
					...
					name[1] == '@' ? st->stack->var_function:// instance/scope variable
					                 &st->script->script_vars;// npc variable
				data->u.num = (long)linkdb_search(n, (void*)reference_getuid(data));
			}
			break;
		case '\'':
			{
...
/*==========================================
 * \83X\83N\83\8A\83v\83g\88ˑ\B6\95ϐ\94\81A\8A֐\94\88ˑ\B6\95ϐ\94\82̉\F0\95\FA
 *------------------------------------------*/
void script_free_vars(struct linkdb_node **node)
{
	...
	while( n != NULL)
	{
		const char* name = get_str((long)(n->key)&0x00ffffff);
		if( is_string_variable(name) )
			aFree(n->data); // \95\B6\8E\9A\8C^\95ϐ\94\82Ȃ̂ŁA\83f\81[\83^\8D폜
		n = n->next;
	}
	linkdb_final( node );
}
...
//
/*==========================================
 * \83R\83}\83\93\83h\82̓ǂݎ\E6\82\E8
 *------------------------------------------*/
c_op get_com(unsigned char *script,int *pos)
{
	int i = 0, j = 0;
...
/*==========================================
 * \83X\83^\83b\83N\82\A9\82\E7\92l\82\F0\8E\E6\82\E8\8Fo\82\B7
 *------------------------------------------*/
...
/*==========================================
 * \8Ew\92\E8\83m\81[\83h\82\F0sleep_db\82\A9\82\E7\8D폜
 *------------------------------------------*/
struct linkdb_node* script_erase_sleepdb(struct linkdb_node *n)
{
	...
	retnode = n->next;
	aFree( n );
	return retnode;		// \8E\9F\82̃m\81[\83h\82\F0\95Ԃ\B7
}

/*==========================================
 * sleep\97p\83^\83C\83}\81[\8A֐\94
 *------------------------------------------*/
int run_script_timer(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	while( node && st->sleep.timer != INVALID_TIMER ) {
		if( (long)node->key == st->oid && ((struct script_state *)node->data)->sleep.timer == st->sleep.timer ) {
...
/*==========================================
 * \83X\83N\83\8A\83v\83g\82̎\C0\8Ds\83\81\83C\83\93\95\94\95\AA
 *------------------------------------------*/
...
void script_cleararray_pc(struct map_session_data* sd, const char* varname, void* value)
{
	...
	else
	{
		for( idx = 0; idx < SCRIPT_MAX_ARRAYSIZE; idx++ )
		{
			pc_setreg(sd, reference_uid(key, idx), (long)value);
		}
	}
}
...
static int32 getarraysize(struct script_state* st, int32 id, int32 idx, int isstring, struct linkdb_node** ref)
{
	...
	else
	{
		for( ; idx < SCRIPT_MAX_ARRAYSIZE; ++idx )
		{
			int64 num = (int64)get_val2(st, reference_uid(id, idx), ref);
			if( num )
				ret = idx + 1;
			script_removetop(st, -1, 0);
		}
...
BUILDIN_FUNC(getitem2)
{
	...
	if(nameid<0) { // \83\89\83\93\83_\83\80
		nameid = -nameid;
		flag = 1;
	}
...
/*==========================================
 *\83L\83\83\83\89\8A֌W\82̃p\83\89\83\81\81[\83^\8E擾
 *------------------------------------------*/
...
/*==========================================
 *\83L\83\83\83\89\8A֌W\82\CCID\8E擾
 *------------------------------------------*/
...
/*==========================================
 *\8Ew\92\E8ID\82\CCPT\90l\90\94\82ƃ\81\83\93\83o\81[ID\8E擾
 *------------------------------------------*/
...
/*==========================================
 *\8Ew\92\E8ID\82̃M\83\8B\83h\96\BC\8E擾
 *------------------------------------------*/
...
/*==========================================
 *\8Ew\92\E8ID\82\CCGuildMaster\96\BC\8E擾
 *------------------------------------------*/
...
/*==========================================
 * \83L\83\83\83\89\83N\83^\82̖\BC\91O
 *------------------------------------------*/
...
/*==========================================
 * \8CĂяo\82\B5\8C\B3\82\CCNPC\8F\EE\95\F1\82\F0\8E擾\82\B7\82\E9
 *------------------------------------------*/
...
/*==========================================
 * \91\95\94\F5\96\BC\95\B6\8E\9A\97\F1\81i\90\B8\98B\83\81\83j\83\85\81[\97p\81j
 *------------------------------------------*/
...
/*==========================================
 * \91\95\94\F5\95i\90\B8\98B\89\\83`\83F\83b\83N
 *------------------------------------------*/
...
/*==========================================
 * \91\95\94\F5\95i\8AӒ\E8\83`\83F\83b\83N
 *------------------------------------------*/
...
/*==========================================
 * \91\95\94\F5\95i\90\B8\98B\90\AC\8C\F7\97\A6
 *------------------------------------------*/
...
BUILDIN_FUNC(failedrefitem)
{
	...
	if(i >= 0) {
		...
		// \90\B8\98B\8E\B8\94s\83G\83t\83F\83N\83g\82̃p\83P\83b\83g
		clif_refine(sd->fd,1,i,sd->status.inventory[i].refine);

		pc_delitem(sd,i,1,0,2,LOG_TYPE_SCRIPT);
		// \91\BC\82̐l\82ɂ\E0\8E\B8\94s\82\F0\92ʒm
...
/*============================================
 * Redu\E7\E3o de refinamento por falha. [Protimus]
 *-------------------------------------------*
...
/*==========================================
 * \83J\83v\83\89\91q\8Cɂ\F0\8AJ\82\AD
 *------------------------------------------*/
...
/*==========================================
 * \83A\83C\83e\83\80\82ɂ\E6\82\E9\83X\83L\83\8B\94\AD\93\AE
 *------------------------------------------*/
...
/*==========================================
 * \83\82\83\93\83X\83^\81[\94\AD\90\B6
 *------------------------------------------*/
...
/*==========================================
 * NPC\8E\E5\91̃C\83x\83\93\83g\8E\C0\8Ds
 *------------------------------------------*/
...
/*==========================================
 * \83C\83x\83\93\83g\83^\83C\83}\81[\92ǉ\C1
 *------------------------------------------*/
...
/*==========================================
 * \83C\83x\83\93\83g\83^\83C\83}\81[\8D폜
 *------------------------------------------*/
...
/*==========================================
 * \83C\83x\83\93\83g\83^\83C\83}\81[\82̃J\83E\83\93\83g\92l\92ǉ\C1
 *------------------------------------------*/
...
/*==========================================
 * NPC\83^\83C\83}\81[\8F\89\8A\FA\89\BB
 *------------------------------------------*/
...
/*==========================================
 * \93V\82̐\BA\83A\83i\83E\83\93\83X
 *------------------------------------------*/
...
/*==========================================
 * \93V\82̐\BA\83A\83i\83E\83\93\83X\81i\93\C1\92\E8\83}\83b\83v\81j
 *------------------------------------------*/
...
/*==========================================
 * \93V\82̐\BA\83A\83i\83E\83\93\83X\81i\93\C1\92\E8\83G\83\8A\83A\81j
 *------------------------------------------*/
...
/*==========================================
 * \83\86\81[\83U\81[\90\94\8F\8A\93\BE
 *------------------------------------------*/
...
/*==========================================
 * \83}\83b\83v\8Ew\92胆\81[\83U\81[\90\94\8F\8A\93\BE
 *------------------------------------------*/
...
/*==========================================
 * \83G\83\8A\83A\8Ew\92胆\81[\83U\81[\90\94\8F\8A\93\BE
 *------------------------------------------*/
...
/*==========================================
 * \83G\83\8A\83A\8Ew\92\E8\83h\83\8D\83b\83v\83A\83C\83e\83\80\90\94\8F\8A\93\BE
 *------------------------------------------*/
...
/*==========================================
 * \89B\82\EA\82Ă\A2\82\E9NPC\82̕\\8E\A6
 *------------------------------------------*/
...
/*==========================================
 * NPC\82\F0\83n\83C\83f\83B\83\93\83O
 *------------------------------------------*/
...
/*==========================================
 * \8F\F3\91Ԉُ\ED\91ϐ\AB\82\F0\8Cv\8EZ\82\B5\82\BD\8Am\97\A6\82\F0\95Ԃ\B7
 *------------------------------------------*/
BUILDIN_FUNC(getscrate)
{
	...
	type=script_getnum(st,2);
	rate=script_getnum(st,3);
	if( script_hasdata(st,4) ) //\8Ew\92肵\82\BD\83L\83\83\83\89\82̑ϐ\AB\82\F0\8Cv\8EZ\82\B7\82\E9
...
/*==========================================
 *\95ߊl\83A\83C\83e\83\80\8Eg\97p
 *------------------------------------------*/
...
/*==========================================
 * \83X\83e\81[\83^\83X\83\8A\83Z\83b\83g
 *------------------------------------------*/
...
BUILDIN_FUNC(globalmes)
{
	...
	mes=script_getstr(st,2);	// \83\81\83b\83Z\81[\83W\82̎擾
	if(mes==NULL) return 0;

	if(script_hasdata(st,3)){	// NPC\96\BC\82̎擾(123#456)
		name=script_getstr(st,3);
	} else {
		name=nd->name;
	}

	npc_globalmessage(name,mes);	// \83O\83\8D\81[\83o\83\8B\83\81\83b\83Z\81[\83W\91\97\90M
...
/* =====================================================================
 * \83M\83\8B\83h\8F\EE\95\F1\82\F0\97v\8B\81\82\B7\82\E9
 * ---------------------------------------------------------------------*/
...
BUILDIN_FUNC(successremovecards)
{
	...
	for( c = sd->inventory_data[i]->slot - 1; c >= 0; --c )
	{
		...
		{// extract this card from the item
			...
			if((flag=pc_additem(sd,&item_tmp,1,LOG_TYPE_SCRIPT))){	// \8E\9D\82ĂȂ\A2\82Ȃ\E7\83h\83\8D\83b\83v
				clif_additem(sd,0,0,flag);
	...
	if(cardflag == 1)
	{	// \83J\81[\83h\82\F0\8E\E6\82菜\82\A2\82\BD\83A\83C\83e\83\80\8F\8A\93\BE
		int flag;
		struct item item_tmp;
		...
		if((flag=pc_additem(sd,&item_tmp,1,LOG_TYPE_SCRIPT))){	// \82\E0\82ĂȂ\A2\82Ȃ\E7\83h\83\8D\83b\83v
			clif_additem(sd,0,0,flag);
			map_addflooritem(&item_tmp,1,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0);
		}
...
BUILDIN_FUNC(failedremovecards)
{
	...
	if(cardflag == 1)
	{
		if(typefail == 0 || typefail == 2){	// \95\90\8B\8E\B8
			pc_delitem(sd,i,1,0,2,LOG_TYPE_SCRIPT);
		}
		if(typefail == 1){	// \83J\81[\83h\82̂ݑ\B9\8E\B8\81i\95\90\8B\EF\82\F0\95Ԃ\B7\81j
			int flag;
			struct item item_tmp;
			item_tmp.id=0,item_tmp.nameid=sd->status.inventory[i].nameid;
			item_tmp.equip=0,item_tmp.identify=1,item_tmp.refine=sd->status.inventory[i].refine;
...
/*==========================================
 * PC\82̏\8A\8E\9D\95i\8F\EE\95\F1\93ǂݎ\E6\82\E8
 *------------------------------------------*/
...
/*==========================================
 * NPC\83N\83\89\83X\83`\83F\83\93\83W
 * class\82͕ς\ED\82肽\82\A2class
 * type\82͒ʏ\ED0\82Ȃ̂\A9\82ȁH
 *------------------------------------------*/
...
/*==========================================
 * NPC\82\A9\82甭\90\B6\82\B7\82\E9\83G\83t\83F\83N\83g
 *------------------------------------------*/
...
/*==========================================
 * \83T\83E\83\93\83h\83G\83t\83F\83N\83g
 *------------------------------------------*/
 ...
 * Verifica se o jogador est\E1 sobre efeito do status especificado.
 *---------------------------------------------------------------*/
...
/*==========================================
 * Cria Runas para o Cavaleiro R\FAnico
 *------------------------------------------*/
