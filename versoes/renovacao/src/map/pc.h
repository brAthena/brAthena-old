...
#define pc_maxparameter(sd)   ( ((sd)->class_&JOBL_THIRD ? ((sd)->class_&JOBL_BABY ? battle_config.max_baby_3rd_parameter : battle_config.max_3rd_parameter) : ((sd)->class_&JOBL_BABY ? battle_config.max_baby_parameter : battle_config.max_parameter)) )
...
#define pcdb_checkid(class_) (class_ < JOB_MAX_BASIC || (class_ >= JOB_NOVICE_T && class_ <= JOB_DARK_COLLECTOR) || (class_ >= JOB_RUNE_KNIGHT && class_ <= JOB_MECHANIC_MADO_T) || (class_ >= JOB_BABY_RUNE && class_ <= JOB_BABY_MECHANIC_MADO) || (class_ >= JOB_SUPER_NOVICE_E && class_ <= JOB_SUPER_BABY_E) || (class_ >= JOB_OBORO && class_ < JOB_MAX))
