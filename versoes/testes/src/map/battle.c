 ...
 **/
		...
		if( sc->data[SC__DEADLYINFECT] && flag&BF_SHORT && damage > 0 && rnd()%100 < 45 + 5 * sc->data[SC__DEADLYINFECT]->val1 )
		...
		else
		{
			switch( skill_num )
			{
				...
				case RK_CRUSHSTRIKE:
					...
					break;
				case RK_STORMBLAST:
					skillratio = 100 * (sd ? pc_checkskill(sd,RK_RUNEMASTERY) : 1) +  100 * (sstatus->int_ / 8);
					break;
				/**
				 * GC Guilotine Cross
				 **/
				...
				case LG_BANISHINGPOINT:
					...
					break;
				case LG_PINPOINTATTACK:
					skillratio += (100 * skill_lv + (5 * status_get_agi(src))) * s_base_level / 120;
					break;
					...
					if( sd )
					{
						switch( sd->itemid )
						{
							...
							case 13264:
								skillratio += 1900;
								break;
							case 13265: skillratio -= 75; break;
							case 13266: skillratio -= 25; break;
							case 13267: skillratio += 100; break;
						}
					}
					else
						skillratio += 300;
			...
			int min_damage, max_damage;
				...
				switch(skill_num){
					...
					case WL_CHAINLIGHTNING_ATK:
						skillratio += 400 + 100*skill_lv;
						if(s_base_level > 100)
							skillratio += skillratio*s_base_level/140;
						break;
					case WL_EARTHSTRAIN:
...
struct Damage battle_calc_misc_attack(struct block_list *src,struct block_list *target,int skill_num,int skill_lv,int mflag)
{
	...
	case CR_ACIDDEMONSTRATION: 
	#ifndef RENEWAL
	//Fórmula = (ATK*0.07*VIT) * WpnSizePnlty + (MATK*0.07*VIT)
		{
			struct Damage wd = battle_calc_weapon_attack(src,target,skill_num,skill_lv,mflag);
			md.damage = wd.damage;
	 ...
	 **/
	case RA_CLUSTERBOMB:
		md.damage += (int)((float)((skill_lv*sstatus->dex) + (5*sstatus->int_))*(1.5 + s_base_level/100)*(20*pc_checkskill(sd,RA_RESEARCHTRAP)/50));
		break;
	case RA_FIRINGTRAP:
 	case RA_ICEBOUNDTRAP:
