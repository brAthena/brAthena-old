...
ACMD_FUNC(jobchange)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1)
	{
		...
		if (!found) {
			...
			//clif_displaymessage(fd, "---- Customizações & Especiais ----");
			//clif_displaymessage(fd, "  22 Casamento            26 Natal         27 Verão           4048 Mestre Taekwon (Union)");
			return -1;
		}
	}

	if (job == 13 || job == 21 || job == 22 || job == 26 || job == 27 || job == 4014 || job == 4022 || job == 4036 || job == 4044 || job == 4048
		 || (job >= JOB_RUNE_KNIGHT_DRAGON && job <= JOB_MECHANIC_MADO_T) || (job >= JOB_BABY_RUNE_DRAGON && job <= JOB_BABY_MECHANIC_MADO)
	) // Deny direct transformation into dummy jobs
		{clif_displaymessage(fd, "Você não pode se transformar para essa classe por um comando de GM.");
		return 0;}
...
ACMD_FUNC(skilltree)
{
	...
	sprintf(atcmd_output, "Jogador está usando árvore de habilidades de %s (%d pontos)", job_name(c), pc_checkskill(pl_sd, NV_BASIC));
	clif_displaymessage(fd, atcmd_output);
