...
static inline unsigned short status_base_status_matk(const struct status_data* status, int level)
{
#ifdef RENEWAL
	return status->int_ + status->int_/2 + (level/4) + (status->dex/5) + (status->luk/3);
#else
	...
	{	//Max HP setting from Summon Flora/marine Sphere
		...
		{	// different levels of HP according to skill level
			...
			if (ud->skillid == NC_SILVERSNIPER) {
				status->max_hp = (status->max_hp / 3) + (1000 * ud->skilllv) + 12 * status_get_lv(mbl);
				status->rhw.atk = status->rhw.atk2 = 200 + 100 * ud->skilllv;
			}
			if (ud->skillid == NC_MAGICDECOY) {
				status->max_hp = (status->max_sp * 4) + (1000 * ud->skilllv) + 12 * status_get_lv(mbl);
...
static unsigned int status_calc_maxsp(struct block_list *bl, struct status_change *sc, unsigned int maxsp)
{
	...
	if(sc->data[SC_VITATA_500])
		maxsp += maxsp * sc->data[SC_VITATA_500]->val1/400;
	if(sc->data[SC_EARTH_INSIGNIA] && sc->data[SC_EARTH_INSIGNIA]->val1 == 3)
		maxsp += 50;
...
int status_change_start(struct block_list* bl,enum sc_type type,int rate,int val1,int val2,int val3,int val4,int tick,int flag)
{
	...
	switch(type)
	{
		...
		case SC_MERC_QUICKEN:
			...
			break;
		case SC_SPEARQUICKEN:
			val2 = val1*10+200; // Adição de ASPD
			val3 = val1*30; // Adição de Crítico
			val4 = val1*2; // Adição de Esquiva
			break;
		case SC_DANCING:
			//val1 : Skill ID + LV
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC__INVISIBILITY:
		if( --(sce->val4) >= 0 )
		{
			if( !status_charge(bl, 0, (status->sp * 6 - sce->val1) / 100) )
				break;
			sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
			return 0;
		}
...
int status_change_timer_sub(struct block_list* bl, va_list ap)
{
	...
	case SC_SIGHTBLASTER:
		if (battle_check_target( src, bl, BCT_ENEMY ) > 0 &&
			status_check_skilluse(src, bl, WZ_SIGHTBLASTER, sce->val1))
		{
...
int status_readdb(void)
{
	...
	sv_readdb(db_path, DBPATH"job_db1.txt",   ',', 6+MAX_WEAPON_TYPE, 6+MAX_WEAPON_TYPE, -1,                            &status_readdb_job1);
	sv_readdb(db_path, DBPATH"job_db2.txt",   ',', 1,                 1+MAX_LEVEL,       -1,                            &status_readdb_job2);
