...
static bool mob_ai_sub_hard(struct mob_data *md, unsigned int tick)
{
	...
	md->sc.data[SC_BLADESTOP] || md->sc.data[SC_DEEPSLEEP] || md->sc.data[SC__MANHOLE] || md->sc.data[SC_CURSEDCIRCLE_TARGET] )
...
{
	const char* filename[] = { DBPATH"mob_db.txt", "mob_db2.txt" };
	int fi;
...
static void mob_readskilldb(void) {
	const char* filename[] = { DBPATH"mob_skill_db.txt", "mob_skill_db2.txt" };
	int fi;
